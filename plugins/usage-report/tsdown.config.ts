/**
 * dsh-plugin-usage-report — 用量报表 DSH 插件。
 *
 * 构建产出两个半区：
 *  - lib/index.js   host 半区（Node ESM：/usage 命令、HTTP 路由、对账与持久化）
 *  - lib/client.js  browser 半区（Web GUI 侧：设置 > 插件 的"用量"页签）
 *
...
const ID = '@dsh-plugin/usage-report'
